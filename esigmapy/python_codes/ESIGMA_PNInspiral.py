"""
ESIGMA_PNInspiral.py
====================
Python translation of ENIGMA_PNInspiral.c
...
* M_PI       -> math.pi
* M_PI2      -> math.pi**2  (macro used in original)
* M_PI3      -> math.pi**3
...
def phi_e_rad(e: float) -> float:
    ef   = 1.0 - e * e
    e2   = e * e
    pre  = 192.0 * sqrt(ef) / (985.0 * e2)
    return pre * (sqrt(ef) * phi_e(e) - phi_e_tilde(e))


def psi_e_rad(e: float) -> float:
    ef   = 1.0 - e * e
    e2   = e * e
    pf1  = 18816.0 / (55691.0 * e2 * sqrt(ef))
    pf2  = 16382.0 * sqrt(ef) / (55691.0 * e2)
    b1   = sqrt(ef) * (1.0 - (11.0 / 7.0) * e2) * phi_e(e) - (1.0 - (3.0 / 7.0) * e2) * phi_e_tilde(e)
    b2   = sqrt(ef) * psi_e(e) - psi_e_tilde(e)
...
def zed_e_rad(e: float) -> float:
    ef   = 1.0 - e * e
    e2   = e * e
    pf1  = 924.0 / (19067.0 * e2 * sqrt(ef))
    pf2  = 12243.0 * sqrt(ef) / (76268.0 * e2)
    b1   = -ef * sqrt(ef) * phi_e(e) + (1.0 - (5.0 / 11.0) * e2) * phi_e_tilde(e)
    b2   = sqrt(ef) * zed_e(e) - zed_e_tilde(e)
    return pf1 * b1 + pf2 * b2
...
def kappa_e_rad(e: float) -> float:
    ...
    pf   = sqrt(ef) / e2
    b1   = sqrt(ef) * kappa_e(e) - kappa_e_tilde(e)
    return pf * b1 / den
...
def x_dot_hereditary_1_5(e: float, eta: float, x: float) -> float:
    ...
    pre = eta * x**6 * sqrt(x)
    if e:
        return pre * 256.0 * pi * phi_e(e) / 5.0
    else:
        return pre * 256.0 * pi / 5.0
...
def x_dot_hereditary_2_5(e: float, eta: float, x: float) -> float:
    ...
    if e:
        b1 = 256.0 * pi * phi_e(e) / ef
        b2 = (-17599.0 * pi * psi_n(e) / 35.0
              - 2268.0 * eta * pi * zed_n(e) / 5.0
              - 788.0 * e2 * pi * phi_e_rad(e) / ef2)
        return pre * (b1 + 2.0*b2/3.0)
    else:
        b1e0 = 256.0 * pi
        b2e0 = -17599.0 * pi / 35.0 - 2268.0 * eta * pi / 5.0
...
def x_dot_hereditary_3(e: float, eta: float, x: float) -> float:
    ...
    if e:
        x_3_term = (64.0 *
                    (-116761.0 * kappa_e(e) +
                     (19600.0*pi2 - 59920.0*EULER_GAMMA - 59920.0*LOG4 - 89880.0*log(x)) * f_e(e))
                    ) / 18375.0
        return pre * x_3_term
    else:
        x_3_term_e0 = (64.0 *
                  (-59920.0*EULER_GAMMA - 116761.0 + 19600.0*pi2 - 59920.0*LOG4 - 89880.0*log(x))
                  ) / 18375.0
...
def x_dot_3pn(e: float, eta: float, x: float) -> float:
    ...
    bit_7 = -40677120.0 * sqrt_ef * (
        ...
        LOG2 - log((1.0 / ef) + (1.0 / sqrt_ef)) - log(x)
    )
...
def x_dot_4pnSO(e: float, eta: float, m1: float, m2: float,
    ...
    val_e0 = pre * (
        ...
        pi * (307708*m1**4*S1z + 307708*m2**4*S2z +
...
def x_dot_4_5_pn(e: float, eta: float, x: float) -> float:
    ...
    val_e0 = pre * (
        451*eta*pi**3/12.0 -
        pi * (700*eta*(3098001198 + eta*(525268513 + 289286988*eta)) +
              145786798080*euler +
...
def e_rad_hereditary_1_5(e: float, eta: float, x: float) -> float:
    ...
    return pre * (-985.0 * pi * phi_e_rad(e) / 48.0)
...
def e_rad_hereditary_2_5(e: float, eta: float, x: float) -> float:
    if not e:
        return 0.0
    pre = 32.0 * eta * e * x**4 * x**4 * sqrt(x) / 5.0
    a2  = (55691.0 * psi_e_rad(e) / 1344.0 + 19067.0 * eta * zed_e_rad(e) / 126.0) * pi
    return pre * a2
...
def e_rad_hereditary_3(e: float, eta: float, x: float) -> float:
    ...
    a4   = (-769.0/96.0) * (16.0*pi**2/3.0 - 1712.0*EULER_GAMMA/105.0 - 1712.0*log(4.0*x32)/105.0) * capital_f_e(e)
    return pre * (a3 + a4)
...
def e_dot_3pn(e: float, eta: float, x: float) -> float:
    ...
    log_term = (
        ...
        * math.log(x * (1.0 + sqrt_e_fact) / (2.0 * e_fact))
    )
...
def l_dot_3pn(e: float, eta: float) -> float:
    ...
    return pre * (t0 + t2 + t4 + sqrt(ef)*(r0 + r2 + r4))
...
def phi_dot_1pn(e: float, eta: float, u: float) -> float:
    """Eq. (A12)"""
    cf = cosu_factor(e, u)
    return -(e*(eta - 4.0)*(e - cos(u))) / (sqrt(1.0 - e*e) * cf**3)
...
def phi_dot_1_5_pnSO_ecc(e: float, m1: float, m2: float,
    ...
    if not e:
        return 0.0
    cf = -1.0 + e * cos(u)
    return (2*e*(m1*S1z + m2*S2z)*(e - cos(u))) / ((-1+e*e)*(m1+m2)*cf**3)
...
def phi_dot_2pn(e: float, eta: float, u: float) -> float:
    ...
    pre = 1.0 / (12.0 * sqrt(ef) * ef * cf5)
    return pre * (t0+t2+t4+t6+c1+c2+c3 + sqrt(ef)*(r0+rc1+rc2+rc3))
...
def phi_dot_3pn(e: float, eta: float, u: float) -> float:
    ...
    pre_factor = 1.0 / (13440.0 * sqrt(e_factor) * e_factor ** 2 * u_factor_pow_7)
...
def rel_sep_1pn(e: float, u: float, eta: float) -> float:
    ef   = 1.0 - e*e
    b1   = 2.0*(1.0 - e*cos(u)) / ef
    b2   = (-18.0 + 2.0*eta - e*(6.0 - 7.0*eta)*cos(u)) / 6.0
    return b1 + b2
...
def rel_sep_1_5pn(e: float, u: float, m1: float, m2: float,
    ...
    return (
        ...
        (2*m1*m1*(S1z + 3*e2*S1z) +
         ...
         2*e*(4*m1*m1*S1z + 4*m2*m2*S2z + 3*m1*m2*(S1z+S2z))*cos(u))
        / (ef**1.5 * M**2)
    )
...
def rel_sep_2pn(e: float, u: float, eta: float) -> float:
    ...
    e2    = e*e
    ef    = 1.0 - e2
    n1    = (-48.0 + 28.0*eta + e2*(-51.0+26.0*eta))*(-1.0+e*cos(u))
    d1    = 6.0*ef**2
    n2    = (72.0*(-4.0+7.0*eta) +
             36.0*sqrt(ef)*(-5.0+2.0*eta)*(2.0+e*cos(u)) +
             ef*(72.0+30.0*eta+8.0*eta2 + e*(-72.0+7*(33.0-5.0*eta)*eta)*cos(u)))
    d2    = 72.0*ef
    return n1/d1 + n2/d2
    ...
    return (
        (2*(-1+e2)**2 *
         (-12*m1**4*S1z - 12*m2**4*S2z -
          21*m1*m1*m2*m2*(S1z+S2z) -
          2*m1**3*m2*(13*S1z+3*S2z) -
          2*m1*m2**3*(3*S1z+13*S2z)) * (2+e*cos(u)) +
         2*ef_sqrt *
         (12*(2+7*e2+e4)*m1**4*S1z +
          ...
          e*(60*(1+e2)*m1**4*S1z + 60*(1+e2)*m2**4*S2z +
             ...
             m1*m2**3*(30*S1z+45*e2*S1z+133*S2z+137*e2*S2z))*cos(u)))
    ) / (6.0 * (-1+e2)**3 * M4)
